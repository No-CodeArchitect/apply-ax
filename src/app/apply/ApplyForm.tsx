...
import { TASKS, DOC_TYPES } from "@/lib/tasks";
import {
  formatBizRegNo,
  normalizeBizRegNo,
  isValidBizRegNo,
...
export default function ApplyForm({ guides }: { guides: Guide[] }) {
  ...
  const [bizState, setBizState] = useState<{
    ...
    exists: boolean;
    appliedTasks: string[];
  }>({ checking: false, valid: false, exists: false, appliedTasks: [] });
  ...
  const fileRefs = {
    ...
    biz_license: useRef<HTMLInputElement>(null),
    track_record: useRef<HTMLInputElement>(null),
  } as const;
  ...
  // 사업자등록번호 등록 여부 조회 (디바운스)
  useEffect(() => {
    const n = normalizeBizRegNo(biz);
    if (!isValidBizRegNo(n)) {
      setBizState({ checking: false, valid: false, exists: false, appliedTasks: [] });
      return;
    }
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setBizState({
          ...
          valid: true,
          exists: !!data.exists,
          appliedTasks: data.appliedTasks || [],
        });
      } catch {
  ...
  const errors = useMemo(() => {
    ...
    if (tasks.length === 0) e.tasks = "지원 과제를 최소 1개 선택하세요.";
    ...
    return e;
  }, [tasks, biz, f, password, passwordConfirm, bizState]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    for (const dt of DOC_TYPES) {
      const file = fileRefs[dt.key as keyof typeof fileRefs].current?.files?.[0];
    ...
    <form onSubmit={onSubmit} className="mt-8 space-y-8" noValidate>
      ...
      <section className="card p-6">
        <h2 className="text-base font-semibold text-navy-800">
          <span className="req">지원 과제 선택</span>
        </h2>
        <p className="mt-1 text-xs text-slate-500">복수 선택이 가능하며, 과제별로 개별 접수됩니다.</p>
        <div className="mt-4 space-y-3">
          {guides.map((g) => {
            const already = bizState.appliedTasks.includes(g.title);
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {already && (
                        <span className="badge bg-amber-100 text-amber-700">접수됨</span>
                      )}
                    </div>
                    {checked && g.guide && (
                      <p className="mt-2 text-xs leading-relaxed text-slate-600">{g.guide}</p>
                    )}
                  </div>
                </div>
              </label>
        ...
        {showErr("tasks") && <p className="field-error">{showErr("tasks")}</p>}
        {existingBiz && bizState.appliedTasks.length > 0 && (
          <p className="mt-3 text-xs text-amber-700">
            이미 접수된 과제는 이 화면에서 다시 접수할 수 없습니다.{" "}
            <Link href="/lookup" className="underline">접수 수정 페이지</Link>를 이용하세요.
          </p>
        )}
      </section>
      ...
      <section className="card p-6">
        ...
        <p className="mt-1 text-xs text-slate-500">
          PDF / HWP / HWPX / DOC / DOCX · 파일당 최대 20MB
        </p>
        <div className="mt-4 space-y-4">
          {DOC_TYPES.map((dt) => (
            <div key={dt.key}>
              ...
              <input
                ref={fileRefs[dt.key as keyof typeof fileRefs]}
                type="file"
